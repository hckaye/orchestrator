...
Cursor Grok 4.7 and Grok CLI Grok 4.7 are separate providers with independent parallel capacity, so both may be dispatched in the same tier. Both use `medium` in the routine tier and `xhigh` in the middle tier. Cursor Grok and Grok CLI have no orchestrator-wide parallel limit. Normally keep Devin implementation workers at five or fewer across the entire orchestrator, including workers belonging to other projects on the same machine. Reviewer use is unlimited, and brief overlap above five is allowed while work is finishing, being reviewed, or handed off; five is an operating target rather than a CLI-enforced hard cap. Devin runs SWE-2 at `high` effort by default, with GLM 5.2 as the second option. OpenCode contributes three worker types over the single `opencode` binary: `opencode` takes a full `provider/model` id, while `opencode-go` and `zen` pin the OpenCode Go (`opencode-go/…`) and OpenCode Zen (`opencode/…`) providers and take a bare model id. All three default to DeepSeek V4.1 Flash (routine tier) with GLM-5.3-Flash as the second option.
